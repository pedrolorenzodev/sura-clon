...
export function LeaderboardPodiumDesktop({ className }: { className?: string }) {
  return (
      ...
      {podium.map((entry) => {
        ...
        return (
          <li
            key={entry.id}
            className={cn(
              "flex h-podium-card min-w-0 flex-1 items-center gap-4 rounded-lg p-4 ring-1 ring-inset",
              GRADIENT[rank],
              style.ring,
          ...
          >
            ...
            <div className="flex min-w-0 flex-col gap-2">
              <div className="flex min-w-0 flex-col">
                ...
                <p className="truncate text-3xs text-foreground">{entry.levelLabel}</p>
              </div>
